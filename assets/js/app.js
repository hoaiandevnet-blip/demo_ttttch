 ...
 if(!isAdminUser()){
  ...
  container.innerHTML=`<div class="staff-self-card">
   ...
   <div>
    ...
    <p><i data-lucide="id-card"></i>Số hiệu: ${escapeHtml(account.u)}</p>
    <p><i data-lucide="phone"></i>SĐT: ${escapeHtml(personal['Số điện thoại']||account.phone||'Chưa cập nhật')}</p>
    <p><i data-lucide="building-2"></i>${escapeHtml(work['Đơn vị công tác']||account.team||'Chưa cập nhật đội')}</p>
    <span class="badge ${account.status==='Hoạt động'?'ok':'lock'}"><i data-lucide="${account.status==='Hoạt động'?'check-circle':'lock'}"></i>${escapeHtml(account.status)}</span>
 ...
 $('#staffProfileDetail').innerHTML=`
  <div class="profile-head">
   ...
   <div>
    ...
    <span class="badge ${account.status==='Hoạt động'?'ok':'lock'}"><i data-lucide="${account.status==='Hoạt động'?'check-circle':'lock'}"></i>${escapeHtml(account.status)}</span>
    <div class="detail-row"><span>Tài khoản</span><span>${escapeHtml(account.u)}</span></div>
    <div class="detail-row"><span>Vai trò</span><span>${escapeHtml(account.role)}</span></div>
...
function renderAssetRows(){
 ...
 const visibleAssets=assets.filter(a=>{
  const belongsToUser=isAdminUser() || a.owner===currentUser?.name;
  return belongsToUser && JSON.stringify(a).toLowerCase().includes(q);
 });
